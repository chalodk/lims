-- 021: RPC csx_company_usage_stats para /admin/uso.
-- SECURITY DEFINER + is_csx(). No abre SELECT de samples/reports a CSX
-- (eso mezclaba Informes/Dashboard de todos los labs).
-- Si ya aplicaste una versión anterior, este archivo DROPEA esas policies.
-- is_csx() ya existe (009). Idempotente.
--
-- Orden: 2/4. Después de 019. Incluye samples.by_type / reports.by_type.
-- Si ya aplicaste una 021 anterior, corre 023_csx_usage_by_type.sql.
-- Probar: CSX /admin/uso con un lab con muestras muestra conteos;
-- CSX Informes NO lista el tenant entero.

DROP POLICY IF EXISTS "CSX can view all clients" ON public.clients;
DROP POLICY IF EXISTS "CSX can view all samples" ON public.samples;
DROP POLICY IF EXISTS "CSX can view all reports" ON public.reports;
